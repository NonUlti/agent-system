...
│   └── marketplace.json   # plugins: [{ name, source: "./", version }]
├── skills/
│   └── <스킬명>/SKILL.md   # 시드 스킬 1개
├── commands/
│   └── <커맨드명>.md       # 시드 커맨드 1개
├── hooks/
...
버전이 plugin.json과 marketplace.json **두 곳에 중복** 존재한다.
릴리스 시 `claude plugin tag` 명령이 두 파일의 일치를 검증하고 `{이름}--v{버전}` git 태그를 만들어 주므로, 릴리스 절차에 반드시 포함한다.

## 시드 콘텐츠 (최소 동작 검증용)

1. **스킬 2개 + 커맨드 2개** — `save-context` / `load-context` (세션 핸드오프).
...
1. 로컬에서 뼈대 생성 (`claude plugin init` 스캐폴딩 또는 수동 생성)
2. 시드 콘텐츠 작성 (스킬 1 + 커맨드 1 + 빈 hooks.json)
3. `claude plugin validate .` 로 매니페스트 검증
4. 로컬 설치 테스트 — 새 세션에서 스킬·커맨드가 뜨는지 확인
5. GitHub public repo 생성 → push
6. `/plugin marketplace add <github-id>/<repo>` → `/plugin install <이름>@<이름>` (user scope)
...
- 설치 후 새 세션에서: 스킬 목록 노출, `/커맨드` 실행 가능 확인
- `claude plugin details <이름>` 으로 구성요소 인벤토리 확인
