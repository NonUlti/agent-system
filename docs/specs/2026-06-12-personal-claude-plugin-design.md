...
수정 → 버전 bump (두 json 모두) → `claude plugin validate . --strict` → `git commit` → `git push` (브랜치 — `source: "./"`가 기본 브랜치를 추적하므로 **이 push가 실제 전파 경로**) → (선택) `claude plugin tag --push`로 두 매니페스트 버전 일치 검증 + `agent-system--v<버전>` 릴리스 태깅 → 각 머신에서 `/plugin marketplace update agent-system` → `/plugin update agent-system` → 재시작
